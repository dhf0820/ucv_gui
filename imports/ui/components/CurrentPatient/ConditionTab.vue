<template>
  <b-card-text>
    ...
    <template v-else-if="currentPatient && currentPatient.conditions && currentPatient.conditions.length > 0">
      <b-modal id="condition-modal" title="FHIR" size="xl">
        <pre>
          {{ modalData }}
        </pre>
      ...
      <!-- <div v-for="entry in currentPatient?.conditions" :key="entry.id">
        <b-row>
          <b-col cols="11">
      ...
      <template v-if="currentPatient">
        <b-table :items="currentPatient.conditions" :fields="fields" responsive="sm" outlined hover
...
export default {
  ...
  computed: {
    ...
    queryCache: ({ $store }) => $store.state.system?.queryCache?.condition,
  ...
  watch: {
    ...
    currentTab(value) {
      console.log("condition current tab", value);
      if (value === 3) {
  ...
  async mounted() {
    // console.log("refs", this.currentPatient?.conditions);
    // if (
    //   this.currentPatient.conditions === null ||
    //   this.currentPatient.conditions === undefined
    // )
    this.loading = true;
    this.BASE_QUERY = `/Condition?patient=${this.currentPatient.id}&_count=10`;
    this.query = `/Condition?patient=${this.currentPatient.id}_count=10`;
  ...
  methods: {
    ...
    async handleSave() {
      const postPatient = (coreUrl, headers) => {
        return new Promise((resolver, reject) => {
          Meteor.call('saveResource', { coreUrl, resourceType: 'Condition', dest: this.$store.state?.isPracticeActive ? this.$store.state?.system?.id : this.$store.state?.practiceSystem?.id, resourceId: this.selectedResourceId, headers }, (error, result) => {
      ...
      try {
        ...
        console.log(res)
        if (!res.issue) {
          this.$vToastify.success('Condition saved successfully')
        } else {
    ...
    async fetchData() {
      try {
        this.loading = true;
        const getCondition = function (coreUrl, query, headers) {
        ...
        const data = await getCondition(this.$store.state.coreUrl, this.query, this.$store.state.headers);
        ...
        const _rows = responseCleaner(bundle.entry, "Condition");
        ...
        console.log("error in conditions", e);
      } finally {
        this.loading = false;
      }
    },
    async handleCachePage() {
      try {
        console.log("condition page cache", this.queryCache);
